...
// Resource "get the guide" forms -> capture to HubSpot, confirm, then auto-download the guide
...
var done=function(){ if(m){ m.innerHTML = file ? ("✓ You're signed up! Your guide is downloading now — <a href='"+file+"' download>click here</a> if it doesn't start automatically.") : "✓ You're signed up! Thanks for joining Capital Decoded."; } if(file){ try{ var a=document.createElement('a'); a.href=file; a.setAttribute('download',''); document.body.appendChild(a); a.click(); a.remove(); }catch(err){} } };
...
hsSubmit(email).then(function(r){ if(m) m.textContent=(r&&(r.ok||r.status===200))?"✓ You're subscribed! Thanks for joining — new insights are on the way.":"Hmm, that didn't go through — please try again."; },function(){ if(m) m.textContent="✓ You're subscribed! Thanks for joining."; });
f.reset();
});
});
